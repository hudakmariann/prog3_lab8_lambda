...
public class Main {
	...
	public static void main(String[] args) {
		...
		/*
		// 2.-3. feladat
		// forras: https://www.techiedelight.com/read-multi-line-input-console-java/
		try (InputStreamReader in = new InputStreamReader(System.in); BufferedReader buffer = new BufferedReader(in)) {
			...
			while ((line = buffer.readLine()) != null) {
				...
				if (cmd[0].equals("exit"))
					...
					System.out.println("list parancs elerve");
					Beer.list(cmd);
				} else if (cmd[0].equals("save")) {
					System.out.println("save parancs elerve");
					Beer.save(cmd);
				} else if (cmd[0].equals("load")) {
					System.out.println("load parancs elerve");
					Beer.load(cmd);
				} else if (cmd[0].equals("find")) {
					System.out.println("find parancs elerve");
					Beer.find(cmd);
				} else if (cmd[0].equals("search")) {
					System.out.println("search parancs elerve");
					Beer.search(cmd);
				} else if (cmd[0].equals("delete")) {
					System.out.println("delete parancs elerve");
					Beer.delete(cmd);
				}
